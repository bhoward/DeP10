...
import static edu.depauw.declan.TokenType.*;
...
public class Scanner {
    ...
    private void scanToken() {
        ...
        case '(':
            if (match('*')) {
                int nest = 1;
                // A comment goes until the matching *).
                while (!isAtEnd()) {
                    if (peek() == '(' && peekNext() == '*') {
                        advance();
                        advance();
                ...
                reporter.error(line, "Unterminated comment.");
                return;
            } else {
                addToken(LEFT_PAREN);
            }
    ...
    private void hexNumber() {
        ...
        if (!match('H')) {
            reporter.error(line, "Expected 'H' at end of hexadecimal literal.");
            return;
        }
    ...
    private void realNumber() {
        ...
        if (match('E')) {
            ...
            if (isDigit(peek())) {
                ...
                reporter.error(line, "Expected digit after 'E' in real literal.");
                return;
            }
        }
