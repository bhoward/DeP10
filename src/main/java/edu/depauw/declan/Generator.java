...
import edu.depauw.declan.ir.IMode;
...
import edu.depauw.declan.resolved.RStmt.Repeat;
import edu.depauw.declan.resolved.RStmt.While;
...
public class Generator implements RExpr.Visitor<Void>, RStmt.Visitor<Void>, RProg.Visitor<Void>, RProc.Visitor<Void> {
    ...
    class BoolGen implements RExpr.Visitor<Void> {
        ...
        public Void visitUnary(Unary expr) {
            switch (expr.op) {
            case LNOT:
                expr.right.accept(new BoolGen(ifFalse, ifTrue));
                break;
            default:
                break;
            }
    ...
    public Void visitRepeat(Repeat stmt) {
        ...
        Label end = newLabel();
        stmt.cond.accept(new BoolGen(end, top));
        generateLabel(end);
